...
⚠️ _I designed most of these code products to be compatible with other languages, but I can't read right-to-left scripts such as Arabic or Hebrew. I can only read the Latin alphabet and Hangul so it's difficult for me to debug in RTL scripts, and I am alone on this project. So although I am pretty confident that left-to-right language packs work well with the theme and scripts, there are still display issues with right-to-left language packs. The UI is very complicated and Firefox doesn't have a singular method for reversing things in RTL mode. Many of the tricks I use to customize Firefox are confounded by RTL inconsistencies. I'm working on improving the RTL mode, but it's currently not in what I would call a user-friendly state. If anyone wants to help with this, I'll welcome PRs and gists that follow the [conventions](#design-conventions)._
