...
This theme requires [**fx-autoconfig**](https://github.com/MrOtherGuy/fx-autoconfig) or some other javascript loader to register the icon package. Download the [resources](/tree/master/resources) folder and place it in your `chrome` folder — fx-autoconfig will automatically register it. For extra completeness, you can download [utils/chrome.manifest](/blob/master/utils/chrome.manifest) to deal with some of the trickier icons. Replace the one from fx-autoconfig with mine, then make sure to rename your `JS` folder to `script`, or change the line in the manifest file. This will strictly redirect some `chrome://` URIs from the vanilla icons to icons from this theme, so the changes will apply globally, even in markup.
...
-   `multiLineUrlbarResults`: When a urlbar result's title overflows off the results panel, this moves its URL to a second line, underneath the title. Results that aren't overflowing are still single lines. This could be done without javascript, but I wanted the URL to be lined up with the title, not with the favicon. This requires some CSS from the end of [uc-urlbar-results.css](/blob/master/uc-urlbar-results.css).
...
-   `restorePreProtonDownloadsButton`: Restores the pre-proton downloads button icons and animations. I kept the new progress animation, but I made it thicker. If you use my theme or my icons you'll definitely want this for the sake of consistency. If you don't use my theme or icons but you still want the old downloads button back, download the *standalone* version instead. (same file name, with `-standalone` at the end) The standalone version has the stylesheet and icons built-in, so doesn't require anything else except a script loader. This version requires [userChrome.au.css](/blob/master/userChrome.au.css) and the [resources/downloads](/tree/master/resources/downloads) folder.

-   `restorePreProtonStarButton`: The bookmark page action button used to have a pretty cool starburst animation. That's been removed but it's not too difficult to restore. The main version of this script requires fx-autoconfig, [userChrome.au.css](/blob/master/userChrome.au.css), and the [resources](/tree/master/resources) folder from my repo. If you don't want to use all that stuff, grab the *standalone* version instead. If you use the standalone version, you won't need any additional CSS and you can use other script loaders instead of fx-autoconfig, but you will need to download the icons from [resources/bookmarks](/tree/master/resources/bookmarks) and place them in `{your profile}/chrome/bookmarks/`. FYI not to state the obvious but this script will have no effect if your browser/OS has `prefers-reduced-motion` enabled.
