...
Adds a new menu to context menus prompted by right-clicking bookmarks, history entries, etc. that allows you to open them in a container tab. This does basically the same thing as the [similarly-named addon](https://addons.mozilla.org/en-US/firefox/addon/open-bookmark-in-container-tab/) by Rob Wu, just by a different method. By doing this with an autoconfig script, we can make the menu appear in a logical order, towards the top of the context menu rather than at the very bottom, where context menu items from addons always go. The menu should be present in *all* context menus where you'd be able to open a bookmark or history entry, including menus, popups, panels, and even the Library/Bookmarks Manager window.
