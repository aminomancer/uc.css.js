...
// @description
/*
The library toolbar button used to have an animation that played when a bookmark was added. It's another casualty of the proton updates. This script restores the library button animation in its entirety, with one small change. The library animation always looked just a tiny bit off for certain window scaling factors — the animation would appear about half a pixel from where the static icon is, causing it to appear to move when the animation finishes.

In order to fix this in my version of the animation, I added some constants at the top of the script (`allowScalingFix` and `forceScalingFix`) that will adjust the icon if enabled. I don't know exactly which scaling factors or OSes have this problem, but 150% scaling on Windows 10 definitely has it. So by default the script will only enable the scaling fix if the OS is Windows and the scaling factor is 1.5. However, if you notice that you have the scaling bug too, you can toggle `forceScalingFix` to true and it will adjust the icon regardless of your OS or scaling factor.

Conversely, if you're using 150% scaling on Windows but for whatever reason the scaling fix is making it look worse somehow, toggling `allowScalingFix` to false will turn the feature off entirely. This version of the script requires fx-autoconfig, userChrome.au.css, and the resources folder from my repo. If you don't all that stuff, grab the [standalone version][] instead.

[standalone version]: https://github.com/aminomancer/uc.css.js/blob/master/JS/restorePreProtonLibraryButton-standalone.uc.js
*/
// @downloadURL    https://cdn.jsdelivr.net/gh/aminomancer/uc.css.js@master/JS/restorePreProtonLibraryButton.uc.js
// @updateURL      https://cdn.jsdelivr.net/gh/aminomancer/uc.css.js@master/JS/restorePreProtonLibraryButton.uc.js
