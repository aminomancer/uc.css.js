...
(function () {
    class HideOnNTP {
        // duskFox has a preference, userChrome.urlbar.hide-bookmarks-button-on-system-pages
        // which hides the urlbar's star button in certain conditions when the tracking protection icon is hidden.
        // that means on the new tab page or on about:blank, there is no star button taking up space in the urlbar,
        // since these pages are unlikely to ever be bookmarked. however, ctrl + D can still open the edit bookmark panel.
        // when this happens, it can't be anchored to the star button since the star button is hidden.
        // so it is anchored to the identity icon instead. this looks kinda silly though because it's not meant to be
        // anchored so far to the left of the star button. so instead if this pref is enabled, we'll try to
        // anchor it to the bookmarks menu button or the library button, if they exist on the user's toolbar.
        _getAnchor(panel) {
                ...
                for (let id of ["star-button-box", "bookmarks-menu-button", "libary-button"]) {
                    ...
                    if (node && !node.hidden) {
                        ...
                        if (bounds.height > 0 && bounds.width > 0) {
                            // add an attribute to the panel if it's going to be anchored to a toolbar button.
                            // duskFox CSS uses this to line it up better with a toolbar button.
                            // normally a panel is positioned -7px vertically from its anchor.
                            // but a panel anchored to a toolbar button is anchored -12px from its anchor.
                            // the reasons for this are just related to how the buttons are flexed in their container.
        ...
        constructor() {
            ...
            gProtectionsHandler.onLocationChange = function onLocationChange() {
                ...
                let homeOrNTP = currentURL === AboutNewTab.newTabURL;
