...
// @description    Modifies some javascript methods so that unread tabs can be styled in CSS, and
// (optionally) adds new items to the tab context menu so you can manually mark tabs as read or
// unread. Normally when you open a new tab without it being selected, it gains the attribute
// "notselectedsinceload" which could be used to style it. But this attribute doesn't go away when
// you select the tab, so it doesn't actually mean "unread." It doesn't go away until you navigate
// to a new page in the tab. But we can change this so it will go away immediately as soon as you
// click it or otherwise select it. Also, normally the attribute isn't added until web progress has
// finished, so unread tab styling won't be applied until after it's finished loading a bit. This
// doesn't look as good so we're also changing it to add the attribute as soon as the tab is
// created. So now all you need to do to style unread tabs is add something like this to your
// userchrome.css file:
// .tabbrowser-tab[notselectedsinceload]:not([pending]:not([busy])) { font-style: italic !important; }
// If you use duskFox (the theme on my repo) you will already have this CSS so there's no need to
// add it. It also includes icons for the context menu items. If you don't use duskFox but you want
// to add context menu icons, the selectors are #context-markAsRead and #context-markAsUnread.
// Search for them in uc-context-menu-icons.css on my repo to see how I do it. Alternatively you
// could modify makeMenuItems() in this script to add "menuitem-iconic" to the classList of the
// menuitems. Then you could add the icon by setting the "image" attribute of the menuitem. I prefer
// not to do it this way because, since the proton context menu update, most built-in context menu
// items do not have icons. So adding icons through the script would make the menuitems stick out
// like a sore thumb for any proton users who don't use my theme.
...
(function () {
    ...
    class UnreadTabsBase {
        ...
        get tabContext() {
            return (
                this._tabContext || (this._tabContext = document.getElementById("tabContextMenu"))
            );
        }
