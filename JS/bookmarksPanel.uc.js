...
class BookmarksPanel {
    ...
    get toolbarButton() {
        return CustomizableUI.getWidget("bookmarks-panel-button")?.forWindow(window)?.node;
    }
    constructor() {
    ...
    makeWidget() {
        if (
            ...
            CustomizableUI.createWidget({
                ...
                onViewShowing: (e) => {
                    if (e.originalTarget !== e.target.ownerGlobal.bookmarksPanel.panelview) return;
                    let panelview = e.target;
                    let body = panelview.body;
                    let showBmMenuContents =
                        this.config["Show bookmarks menu contents instead of recent bookmarks"];
                    ...
                    if (!this.mainViewMenu) {
                        ...
                        bmToolbarSubviewNav.addEventListener("command", (e) => this.showSubView(e));
                        ...
                        // separator
                        toolbarView.appendChild(document.createXULElement("toolbarseparator"));

                        // view bookmarks toolbar
                        this.bmViewToolbar = toolbarView.appendChild(
                            this.create(document, "toolbarbutton", {
                                id: "PanelUI-bookmarks-view-toolbar-subviewbutton",
                                class: "subviewbutton panel-subview-footer-button subviewbutton-iconic",
                        ...
                        bmUnfiledSubviewNav.addEventListener("command", (e) => this.showSubView(e));
                        ...
                        bmMobileSubviewNav.addEventListener("command", (e) => this.showSubView(e));
                        ...
                        if (showBmMenuContents) {
                            let bmRecentSubviewNav = body.appendChild(
                                this.create(document, "toolbarbutton", {
                                    ...
                                    class: "subviewbutton subviewbutton-iconic subviewbutton-nav bookmark-item",
                                    closemenu: "none",
                                    "widget-type": "view",
                            ...
                            this.buildSubView(
                                ...
                                "place:queryType=" +
                                    ...
                                    `&maxResults=50&excludeQueries=1`
                            );
                        }
                    ...
                    this.mainMenu = new PlacesContainerPanelview(
                        ...
                        showBmMenuContents
                            ...
                            : "place:queryType=" +
                              ...
                              "&sort=" +
                              Ci.nsINavHistoryQueryOptions.SORT_BY_DATEADDED_DESCENDING +
                              `&maxResults=${this.config["Max recent bookmarks"]}&excludeQueries=1`
                    );
                    if (showBmMenuContents)
                    ...
                    this.mobileSubview.hidden = !BookmarkingUI._shouldShowMobileBookmarks();
                },
    ...
    buildSubView(placesNode, query) {
        let newId =
            "PanelUI-bookmarks-" + this.makeSafeId(placesNode?.bookmarkGuid || "recentView"); // turn the bookmarks GUID into a DOM node ID
    ...
    showSubView(e, anchor) {
        ...
        PanelUI.showSubView(
            "PanelUI-bookmarks-" +
                this.makeSafeId(anchor._placesNode?.bookmarkGuid || "recentView"),
            anchor,
            e
        );
    }
...
class PlacesContainerPanelview extends PlacesPanelview {
    ...
    _onCommand(e) {
        ...
        else button.removeAttribute("closemenu");
    ...
    _createDOMNodeForPlacesNode(placesNode) {
        ...
        if (type == Ci.nsINavHistoryResultNode.RESULT_TYPE_SEPARATOR)
            ...
            if (type == Ci.nsINavHistoryResultNode.RESULT_TYPE_URI) {
                ...
                if (!this._domNodes.has(placesNode)) this._domNodes.set(placesNode, element);
    ...
    nodeTitleChanged(aPlacesNode, aNewTitle) {
        let elt = this._getDOMNodeForPlacesNode(aPlacesNode);
        elt.setAttribute("label", aNewTitle || PlacesUIUtils.getBestTitle(aPlacesNode));
    }
    ...
    buildSubView(placesNode) {
        ...
        view.addEventListener(
            "ViewShowing",
            (e) => (view.panelMenu = new PlacesContainerPanelview(container, view, placesNode.uri))
        );
