...
// affects bookmark items. When you enable this pref and left-click a bookmark
// (e.g., in the bookmarks toolbar or menu) it opens in a new tab instead of in
// the current tab. But if you left-click a history entry or a synced tab, it
// will still open in the current tab. So you'd have to middle click or
// ctrl+click to avoid losing your current tab's navigation state. This script
// just makes that preference apply to history and synced tabs too. It's
// relatively easy to do this because there isn't any technical reason it
// doesn't work for history or synced tabs. For whatever reason, the command
// handler for clicking these places nodes just intentionally checks if it's a
// bookmark and opens it in a new tab only if it passes the check. I guess this
// is probably because the preference was called "loadBookmarksInTabs" a long
// time ago, so making it affect other items might be confusing, but changing
// the name of a longstanding pref would also be confusing. So I guess Firefox
// is just stuck with the ramifications of a minor mistake somebody ostensibly
// made years ago, which is a perfect use case for an autoconfig script.
